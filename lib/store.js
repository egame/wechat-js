...
RedisStore._getKey = function (key) {
    return 'wechat:store:' + key;
};
...
RedisStore.prototype.set = function (key, value, expire, next) {
    ...
    if (expire) {
        return this._conn.SETEX(this._getKey(key), expire, value, next)
    }
    this._conn.SET(this._getKey(key), value, next);
};

RedisStore.prototype.get = function (key, next) {
    this._conn.GET(this._getKey(key), function (err, reply) {
        if (err) {
            throw Error(err);
        }
        next && next(reply);
